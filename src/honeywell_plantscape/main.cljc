(ns honeywell-plantscape.main
  "Kotodama WASM entrypoint for the Honeywell_plantscape clean-room actor (L4) — Clojure port.
  ...
  facts (`honeywell_plantscape.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "honeywell_plantscape")
(def tier "L4")
...
(def entity-specs
  [{:entity "Tag"        :plural "tags"        :id-prefix "honeywel_tag"
   ...
   {:entity "ControlLoop" :plural "controlloops" :id-prefix "honeywel_con"
    :fields [:name :setpoint :processValue :mode]   :required [:name :setpoint]
    :coerce {:setpoint :float :processValue :float} :refs {}}
   {:entity "Batch"      :plural "batches"     :id-prefix "honeywel_bat"
...
(defn emit-facts
  "EAVT facts for one record: {\"honeywell_plantscape.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "honeywell_plantscape-compat" :tier tier :entities entities} 200])
...
(defn start! [] :honeywell_plantscape-compat/ready)
